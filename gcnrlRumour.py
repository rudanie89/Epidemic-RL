...
env = socialEnv()
observation,_ = env.reset() #numpy.ndarray'
...
class REINFORCE_graph(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        #x = self.layer_norm(x)
        x = global_add_pool(self.layer_norm(x), torch.LongTensor([0 for _ in range(100)]).to(device))
    ...
    def train_net(self, gamma):
        ...
        for r, prob in self.roll_out[::-1]:
            #print("roll_out:", self.roll_out[::-1])
            G_t = r + gamma * G_t
            G.append(G_t)
        G = np.array(G)
        G_mean = G.mean()
...
if __name__ == "__main__":
    ...
    env = socialEnv()
    observation,edgelist_index = env.reset()
    ...
    for epi in range(episodes):
        ...
        infected_train = []
        inext_train = []
        for t in range(20): ##count(): ##tam thoi la nhu vay
            ...
            a = a_distrib.sample()
            #action_a = torch.argmax(torch.exp(a_prob))
            print("action:",a)
            observation, RLreward, done, user_debunker, notin, budgetAll, infectednodeM, infectnext, snext, inext, _ = env.step(a)

            useBud = budgetAll
            print("action and reward at episode " + str(epi) + " at step " + str(t) + " :",
                  [a, user_debunker, notin, RLreward * 100, snext, inext, done])
            reward = RLreward
            recoveredEpi.append((t+1, int(reward*100)))
            action_train.append((t + 1, a))
            ...
            Policy.put_data(( reward, a_prob[0][a]))
            s = next_state
        ...
        #while not done: ## cho nay minh lam nhu bai cu cua minh thoi
