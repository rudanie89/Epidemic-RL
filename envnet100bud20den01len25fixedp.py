...
G = nx.generators.random_graphs.binomial_graph(100,0.1,directed=False) #seed=246,
...
for i in G.nodes:
    ...
    proindegreeDict[str(i)] = round(1/(len(indegreeDict[str(i)])+0.001),6)
...
class socialEnv(gym.Env): #object):
    ...
    def initialstatefunc(self): #,mitigation #,iepisode
        ...
        while len(self.Sinfected) < 5:
            ...
            userState_AtS, DelaterS, MislaterS, DM_allS, RM_laterS, IsetS = update_B(self.G, self.infectednode, [], 25,
            ...
            if ((len(self.Sinfected)>=5) and (len(self.Sinfected)<=40)):
